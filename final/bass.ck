...
fun void bassPlayer() {
    ...
    Math.random2(-2, 2) +=> walkMelody; 
    
    //Technical/Code Score: Use of if/else statements
    if (walkMelody < 0) {
        1 => walkMelody;
    }
    
    if (walkMelody >= sc.Cmaj.cap()) {
        sc.Cmaj.cap() - 2 => walkMelody;
    }
    
    playChord(sc.Cmaj[walkMelody], "major");
